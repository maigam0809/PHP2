...
class BaseModel 
{
	...
	{
        $conn = new PDO('mysql:host=127.0.0.1;dbname=ph10005_mvcphp2;charset=utf8', 'root', '');
        return $conn;
    }
	...
	public function update($arr){
		...
		$this->queryBuilder .= " where book_id = :book_id";
		$stmt = $this->getConnect()
					->prepare($this->queryBuilder);
		foreach ($arr as $key => &$value) {
			$stmt->bindParam(":$key", $value);
	...
	public static function getAtributes($arr){
 	...
 	public static function destroy($id_table,$id){
 		$model = new static();
